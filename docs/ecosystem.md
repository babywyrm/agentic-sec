...
| **[stoneburner](https://github.com/babywyrm/stoneburner)** (v0.12.0) | LLM provider benchmarking (cost, throughput, latency, accuracy via LLM-as-judge) across Claude, OpenAI, Bedrock, Ollama, vLLM, **Groq**, **Together AI**, **Google Gemini**, **llama.cpp**, and **brain-gateway**. **API server mode** (`atomics server`, optional `[api]` extra) with async jobs (`POST /api/v1/runs`, `POST /api/v1/evals`, `GET /api/v1/jobs/{id}`), API key auth, comparison, and recent-run reports. **Distributed benchmark runs** (`atomics distributed run` / `atomics worker`, split-task coordinator with mixed local/remote workers, per-worker API keys). Adversarial resilience testing (**72 fixtures**; `--runs N`, `--extra-judges`, `--compare`, `--json-out`, `--fail-on-resilience`). Red/blue (10 fixtures), `archreview`, live `probe`. **RAG real retrieval** (`rag-index` / `rag --index` / `rag-retrieval`; optional `[rag]` extra). Richer **multi-turn** (35 conversations), **codegen** (15 fixtures, execution-based), multilingual eval, cost **advisor**, webhook notifications. Also: `sweep` / `stress` / `soak` / `scenario` / `qa` / `capacity` / `models`, `inference.env` + `atomics.inference`, judge calibration, token-burn fidelity, OS-keychain secrets, `ARCHITECTURE.md`, ruff + mypy + **1715 tests**. | MCP protocol enforcement, vulnerability scanning, policy generation. Stoneburner complements mcpnuke — it measures LLM reasoning quality and adversarial resilience, not MCP protocol integrity. | `atomics compare --narrative --output`; every suite has `--json-out`; suite-isolated `export`; SQLite results DB (schema v20); HTTP API under `/api/v1/*`; distributed endpoints under `/api/v1/workers` and `/api/v1/distributed/*` |
...
- ✅ **stoneburner v0.12.0** — distributed benchmark runs
- ✅ **stoneburner v0.15.0** — distributed full-mode runs (`--mode full`), npm worker bridge (`atomics worker-npm` with `--runtime node` capability routing and `--pool-size N`), worker-npm CLI coverage backfilled to 94%; schema v20; **2011 tests** — *2026-07-29* (`atomics distributed run` / `atomics worker`, split-task coordinator with mixed local/remote workers, per-worker API keys, heartbeat/requeue/retry/recovery); CLI distributed status emits clean JSON; worker CLI supports `--provider`/`--model`/`--host`; SQLite `ResourceWarning` fixes in tests; schema v20; **1715 tests** — *2026-07-23*
