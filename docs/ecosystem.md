...
| **[mcpnuke](https://github.com/babywyrm/mcpnuke)** (v6.13.0) | Static, behavioral, infrastructure, and exploit-chain scanning of MCP servers. **Dual-stack protocol support** — scans both the legacy handshake and the **2026-07-28 stateless spec** (`--protocol-mode {auto,legacy,stateless}`, SEP-2243 routing headers, `server/discover` probing). Policy recommendation (`--generate-policy`). Teleport-aware checks. Per-lane reporting (spec 2026-04-26). SARIF 2.1.0 export (`--sarif`). Configurable CI severity gate (`--fail-on`). Token redaction in all output paths. 59 registered checks + 24 deep probes, **993 tests**. | Runtime request blocking (that's nullfield's job). Identity issuance. Deployment. | Finding dataclass; `--json` / `--sarif` output; `docs/cli-reference.md` generated from the parser |
...
| **[stoneburner](https://github.com/babywyrm/stoneburner)** (v0.16.0) | LLM provider benchmarking (cost, throughput, latency, accuracy via LLM-as-judge) across Claude, OpenAI, Bedrock, Ollama, vLLM, **Groq**, **Together AI**, **Google Gemini**, **llama.cpp**, and **brain-gateway**. **API server mode** (`atomics server`, optional `[api]` extra) with async jobs (`POST /api/v1/runs`, `POST /api/v1/evals`, `GET /api/v1/jobs/{id}`), API key auth, **per-caller job quotas**, **eval spend ceilings** (`budget_usd` / `--budget`), **correlation IDs** (`X-Request-ID`), **readiness probe** (`GET /api/v1/ready`), comparison, recent-run reports, and an optional **web dashboard** (`--with-dashboard`, `/dashboard`). **Distributed benchmark runs** (`atomics distributed run` / `atomics worker`, split, fleet, and full modes, mixed local/remote workers, per-worker API keys, npm worker bridge with capability routing). Adversarial resilience testing (**72 fixtures**; `--runs N`, `--extra-judges`, `--compare`, `--json-out`, `--fail-on-resilience`). Red/blue (10 fixtures), `archreview`, live `probe`. **RAG real retrieval** (`rag-index` / `rag --index` / `rag-retrieval`; optional `[rag]` extra). Richer **multi-turn** (35 conversations), **codegen** (15 fixtures, execution-based), multilingual eval, cost **advisor**, webhook notifications. Also: `sweep` / `stress` / `soak` / `scenario` / `qa` / `capacity` / `models`, `inference.env` + `atomics.inference`, judge calibration, token-burn fidelity, OS-keychain secrets, `ARCHITECTURE.md`, ruff + mypy + **2267 tests**. | MCP protocol enforcement, vulnerability scanning, policy generation. Stoneburner complements mcpnuke — it measures LLM reasoning quality and adversarial resilience, not MCP protocol integrity. | `atomics compare --narrative --output`; every suite has `--json-out`; suite-isolated `export`; SQLite results DB (schema v20); HTTP API under `/api/v1/*`; distributed endpoints under `/api/v1/workers` and `/api/v1/distributed/*`; dashboard at `/dashboard` |
...
- ✅ **stoneburner v0.7.0** — `atomics archreview` security-architecture repo benchmark (deterministic token-budgeted evidence packs, objective difficulty-weighted OWASP recall/precision against per-repo answer keys, self-judge-guarded reasoning score, multi-round finding-set robustness); reasoning-model judge support (deepseek-r1, phi4-reasoning, gemma4); zero-trust + agentic-reasoning + MCP adversarial fixture suites; schema v15 — *2026-06-23*
- ✅ **stoneburner v0.8.0** — three new adversarial suites (multi-turn manipulation, RAG-poisoning, tool-description-injection) taking the total to **64 fixtures** with a single-source-of-truth registry; `--json-out` / `--compare` / `--fail-on-resilience` CI gate on `adversarial`, `--runs` variance + `--json-out` on `redblue`, suite-isolated `export`; adversarial/probe/archreview SQLite persistence with parent run rows; research: mistral-small:24b 78.2% vs mistral-nemo:12b 61.9% on the 64-fixture suite — *2026-07-04*
- ✅ **stoneburner structure & hardening pass** — `ARCHITECTURE.md` contributor map; `stats.py` + single provider factory (dedup); `py.typed` + mypy gate in CI; security: `secrets get` masked by default, DB backup before schema-migration wipe; eval-suite convergence (`to_dict()`/`--json-out`/parent-run-row across all five suites); 1078 tests — *2026-07-04*
- ✅ **stoneburner v0.11.0** — API server mode (`atomics server`, `[api]` extra, async jobs + `X-API-Key`); real RAG retrieval (`rag-index` / `rag --index` / `rag-retrieval`, `[rag]` extra); richer multi-turn (35 fixtures), codegen + multilingual evals, cost advisor, webhook notifications; new providers Groq / Together / Gemini / llama.cpp; adversarial 64 → **72 fixtures**; schema v20; 1683 tests — *2026-07-20*
- ✅ **stoneburner v0.12.0** — distributed benchmark runs (`atomics distributed run` / `atomics worker`, split-task coordinator, mixed local/remote workers, per-worker API keys, heartbeat/requeue/retry/recovery); CLI distributed status emits clean JSON; worker CLI supports `--provider`/`--model`/`--host`; SQLite `ResourceWarning` fixes in tests; schema v20; **1715 tests** — *2026-07-23*
- ✅ **stoneburner v0.15.0** — distributed full-mode runs (`--mode full`), npm worker bridge (`atomics worker-npm` with `--runtime node` capability routing and `--pool-size N`), worker-npm CLI coverage backfilled to 94%; schema v20; **2011 tests** — *2026-07-29*
- ✅ **stoneburner v0.15.1** — optional web dashboard (`--with-dashboard`, `/dashboard`), distributed job/worker list endpoints (`GET /api/v1/distributed/runs`, `GET /api/v1/workers`), `atomics server --db-path PATH`; schema v20; **2031 tests** — *2026-07-30*
- 🔒 **stoneburner v0.15.2** — security hardening from a project-wide audit: model-generated code sandboxed out of the evaluating process, dashboard XSS fixed, assignment submission bound to the owning worker, worker keys separable from submitter keys (`--worker-api-key`), `--no-auth` restricted to loopback binds; plus bounded job retention, request caps, security headers, and `pip-audit`/gitleaks in CI; schema v20; **2116 tests** — *2026-08-03*
- ✅ **stoneburner v0.16.0** — bounded and observable: spend ceilings on eval suites (`budget_usd` on the API, `--budget` on all twelve eval commands, one shared ceiling across model and judges), per-caller job quotas so one key cannot hold every slot, correlation IDs (`X-Request-ID`) that propagate into async job logs, structured access logging that omits query strings and bodies, and readiness split from liveness (`GET /api/v1/ready`); schema v20; **2267 tests** — *2026-08-04*
- ✅ **mcpnuke MCP 2026-07-28 dual-stack** — scans the stateless spec alongside legacy handshake servers; `--protocol-mode {auto,legacy,stateless}`, `Mcp-Method`/`Mcp-Name`/`MCP-Protocol-Version` routing headers (SEP-2243) with CR/LF stripped, client identity in `params._meta`, `Mcp-Session-Id` and `notifications/initialized` legacy-only (SEP-2567), `server/discover` probing with a Lane 5 / Transport A finding for anonymous capability reads — *2026-08-01*
- ✅ **mcpnuke quality hardening** — `ruff` 370 → 0, `mypy` 81 → 48 on a tightening ratchet with `disallow_untyped_defs` across `core/`, credential and injection regexes consolidated into single sources of truth after five divergent copies were found to disagree, CI green for the first time on Python 3.11/3.12/3.13; **993 tests** — *2026-08-02*
- ✅ **mcpnuke docs restructure** — README cut 1018 → 304 lines into a navigable front door, reference material split into `docs/`, `docs/cli-reference.md` generated from the argument parser so it cannot drift from `--help` — *2026-08-02*
...
| Benchmark providers | [stoneburner](https://github.com/babywyrm/stoneburner) — `atomics run --provider brain-gateway`; API mode via `atomics server` |
