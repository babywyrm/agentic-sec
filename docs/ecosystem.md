...
| **[stoneburner](https://github.com/babywyrm/stoneburner)** (v0.8.0) | LLM provider benchmarking (cost, throughput, latency, accuracy via LLM-as-judge) across Claude, OpenAI, Bedrock, Ollama, **vLLM/OpenAI-compatible gateways**, and **brain-gateway**. Adversarial resilience testing (**64 fixtures** across 8 suites: base, zerotrust, agentic-reasoning, mcp, tool-safety, **multi-turn**, **RAG-poisoning**, **tool-description-injection**; `--runs N` multi-pass variance, `--extra-judges` multi-judge consensus, `--compare` side-by-side diff, `--json-out`, `--fail-on-resilience` CI gate). Red/blue security capability eval (10 fixtures, `--runs N`). Security-architecture repo review (`atomics archreview`, objective OWASP recall/precision + judge reasoning). Live infrastructure probe via `probes.yaml`. Thinking mode benchmarking. Multi-model sweeps (`atomics sweep`), GPU stress + multi-model VRAM contention (`atomics stress`), long-duration stability + named regression baselines (`atomics soak`), mixed-workload simulation (`atomics scenario`), CTF solvability / AI-gate regression (`atomics qa`), user capacity projection (`atomics capacity`), model discovery (`atomics models`). Vendor-neutral `inference.env` standard + agnostic resolver (`atomics.inference`). Judge accuracy & calibration (self-judge guard, gold-criteria coverage) and token-burn fidelity. OS-keychain secrets layer (`atomics secrets`, masked by default). Contributor `ARCHITECTURE.md`; CI enforces ruff + mypy (ships `py.typed`) + 1078 tests. The `brain-gateway` provider routes through camazotz's MCP inference endpoint. | MCP protocol enforcement, vulnerability scanning, policy generation. Stoneburner complements mcpnuke — it measures LLM reasoning quality and adversarial resilience, not MCP protocol integrity. | `atomics compare --narrative --output`; every suite has `--json-out`; suite-isolated `export --suite {eval,redblue,adversarial,...}`; SQLite results DB (schema v15) |
